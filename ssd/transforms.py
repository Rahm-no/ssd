...
class RandomHorizontalFlip(T.RandomHorizontalFlip):
    def forward(self, image: Tensor,
        ...
        size = get_image_size(image)
         # Log transformation details for RandomHorizontalFlip
        flip_times.append({
            "Image ID": image_name,
        ...
        print("rnouaj-RandomHorizontalFlip. Image:", image_name, "size:(", size,") -",int(size[0])*int(size[1]), "time:", end_time*1000, "ms")
        return image, target
...
RandomPhotometricDistort_times=[]
class RandomPhotometricDistort(nn.Module):
    ...
    def forward(self, image: Tensor,
        ...
        image_name = get_image_name(tensor_id)
        size = get_image_size(image)
        RandomPhotometricDistort_times.append({
            "Image ID": image_name,
            "Image Size": size,  # Image size
            "Time (ms)": duration*1000 
        })
        # print("bitchebe-RandomPhotometricDistort. Image: ", image_name, "time: ", end_time*1000, "ms")
        print("rnouaj-RandomPhotometricDistort. Image:", image_name, "size:(", size,") -",int(size[0])*int(size[1]), "time:", end_time*1000, "ms")
        return image, target
...
def save_times_to_excel():
    

    # Convert the list of dictionaries to a pandas DataFrame
    RandomPhotometricDistort_df = pd.DataFrame(RandomPhotometricDistort_times)
    RandomPhotometricDistort_df.to_excel("/datasets/open-images-v6-mlperf/RandomPhotometricDistort_times.xlsx", index=False)

    RandomIOU_df = pd.DataFrame(RandomIOU_times)
    RandomIOU_df.to_excel("/datasets/open-images-v6-mlperf/RandomIOU_times.xlsx", index=False)

    tensor_df = pd.DataFrame(tensor_times)
    tensor_df.to_excel("/datasets/open-images-v6-mlperf/tensor_times.xlsx", index=False)

    flip_df = pd.DataFrame(flip_times)
    flip_df.to_excel("/datasets/open-images-v6-mlperf/flip_times.xlsx", index=False)

    RandomZoomOut_df = pd.DataFrame(RandomZoomOut_times)
    RandomZoomOut_df.to_excel("/datasets/open-images-v6-mlperf/RandomZoomOut_times.xlsx", index=False)
